...
namespace RenderDemo.Pages
{
    ...
    public class WriteableBitmapPage : Control
    {
        ...
        {
            _unpremulBitmap = new WriteableBitmap(new PixelSize(900, 700), new Vector(96, 96), PixelFormat.Bgra8888, AlphaFormat.Unpremul);
        ...
        public override void Render(DrawingContext context)
        {
            ...
            void FillPixelsDirect(WriteableBitmap bitmap, byte fillAlpha, bool premul)
            {
                using (var fb = bitmap.Lock())
                {
                    var data = new int[fb.Size.Width * fb.Size.Height];
                    unsafe
                    {
                        int* ptr = (int*)fb.Address.ToPointer();
                        for (int y = 0; y < fb.Size.Height; y++)
                        {
                            for (int x = 0; x < fb.Size.Width; x++)
                            {
                                ...
                                ptr[y * fb.Size.Width + x] = (int)color.ToUInt32();
                            }
                        }
                    }
                }
            ...
            FillPixelsDirect(_unpremulBitmap, alpha, false);
